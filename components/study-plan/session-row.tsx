...
function ActionButton({ session, onStart }: { session: CalendarSession; onStart: () => void }) {
  ...
  if (session.status === "in_progress") {
    if (session.subjectSlug && session.topicSlug) {
      return (
        <Link
    ...
    <div className="border-border relative flex items-center gap-4 border-b py-3.5 last:border-0">
      ...
      <div className="flex min-w-0 flex-1 items-stretch gap-3">
        ...
        <div className="min-w-0 flex-1 py-0.5">
          <p
            className={`truncate text-sm font-semibold ${done ? "text-text-secondary line-through" : "text-text-primary"}`}
          >
            {session.title}
          </p>
          {session.subjectName && (
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        <ActionButton session={session} onStart={handleStart} />
